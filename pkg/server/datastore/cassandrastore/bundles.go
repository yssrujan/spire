...
const (
	...
	maxCASRetries = 10
	// Initial backoff duration for CAS retries
...
func appendBundle(ctx context.Context, session *gocql.Session, b *common.Bundle, log logrus.FieldLogger) (*common.Bundle, error) {
	...
	for attempt := 0; attempt < maxCASRetries; attempt++ {
		...
		if err == gocql.ErrNotFound {
			...
			if !applied {
				// Another process inserted concurrently, retry
				continue
			}
			return b, nil
		}
		...
		// Increment sequence number for changed bundle
		...
		// Use CAS (Compare-And-Set) to ensure no concurrent modifications
		// We compare against the WRITETIME we read earlier
		now := time.Now()
		applied, err := session.Query(`
			UPDATE bundles
			SET data = ?, updated_at = ?
			WHERE trust_domain = ?
			IF WRITETIME(data) = ?
		`, updatedModel.Data, now, trustDomain, existsToken).
			WithContext(ctx).
			Consistency(gocql.Quorum).
			MapScanCAS(make(map[string]interface{}))
		...
		if !applied {
			...
			if log != nil {
				log.WithField("trust_domain", trustDomain).Debug("CAS failed, bundle was modified concurrently")
			}
			continue
		}
	...
	return nil, status.Errorf(codes.Aborted, "failed to append bundle after %d attempts due to concurrent modifications", maxCASRetries)
}
...
func listBundles(ctx context.Context, session *gocql.Session, req *datastore.ListBundlesRequest) (*datastore.ListBundlesResponse, error) {
	...
	var tokens []int64 // keep parallel slice to get correct last token after trim
	var token int64
	var trustDomain string
	var data []byte
	...
	for iter.Scan(&token, &trustDomain, &data) {
		...
		if err := proto.Unmarshal(data, bundle); err != nil {
			// Skip corrupted entries instead of failing the whole request
			continue
		}
