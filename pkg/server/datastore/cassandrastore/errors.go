...
const (
	datastoreCassandraErrorPrefix  = "datastore-cassandra"
...
func (s *cassandraError) Error() string {
	...
	if s.err != nil {
		return fmt.Sprintf("%s: %s", datastoreCassandraErrorPrefix, s.err)
	}

	return fmt.Sprintf("%s: %s", datastoreCassandraErrorPrefix, s.msg)
}
